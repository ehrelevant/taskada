...
export function useReport() {
  ...
  const checkExistingReport = useCallback(async () => {
    try {
      const exists = await providerClient.checkReportExists(bookingId);
  ...
  const onSubmit = useCallback(
    async (data: ReportFormData) => {
      try {
        const report = await providerClient.createReport({
        ...
        if (selectedImages.length > 0) {
          await providerClient.uploadReportImages(report.id, selectedImages);
        }
